# Rule: Item Specification Sheet (項目仕様 / Item Specification)

## Sheet name keywords

`項目仕様`, `項目情報`, `Item Spec`, `Item Specification`, `Field Spec`, `Spec item`, `Đặc tả trường`, `Danh sách trường`, `Chi tiết item`

## Goal

Field/item specification sheet. Output: metadata + one item table keeping all columns.

## Heading hierarchy

- YAML frontmatter (metadata — see section 1) at the very top of the file, before the H1
- Sheet name = `#` (H1)
- `項目仕様` (Item Specification) section = `##` (H2)
...
## 2. Item Specification table (項目仕様) — Markdown Table with DYNAMIC columns

- Convert the whole table to a Markdown table.
- **Keep the ENTIRE table header** — do NOT hard-code column names, because the header can vary between documents.
- Preserve column order and all rows as in Excel.

Commonly seen columns (for reference only, NOT fixed):
No. / Item Name (項目名) / Type (タイプ) / Item Length (項目長) / Decimal Places (小数) / Display (表示制御) / Display Rule・Notes (表示制御・備考) / Search Condition (検索項目) / CDS ID / CDS Item / ...

## Constraints

- Do NOT drop or rename any column; output the exact header present on the sheet.
- Do NOT omit any row.
